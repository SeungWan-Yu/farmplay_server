...
exports.getToss = async (req,res) => {
    try {
        ...
        if(req.query.oId==undefined){
            ...
            data = await webViewModel.addPayReq(req.query);
...
exports.getTossSuccess = async (req,res) => {
    ...
    function returnAffect(data,type) {
        ...
        if(type=="add"){
            if(data.affectedRows==0){
                msg = "notAffect";
                console.log("영향없음");
                console.log("/v1/webView/tossFail?result="+"dataBase"+"&msg="+msg);
    ...
    try {
        ...
        var data1 = await webViewModel.getPayReq(query); 
        var reqPirce = data1[0].payReqPrice;
        var uId = data1[0].payReqUid;
        query.uId = uId;
        var data2 = await webViewModel.updatePayReqAmount(query); 
        ...
        if(amount==reqPirce){
            console.log("금액같을경우");
            query.state = "결제승인대기";
            var data3 = await webViewModel.updatePayReqState(query); 
            ...
            if(response.result=="success"){
                query.state = "결제완료";
                console.log("체크하깅");
                var data4 = await webViewModel.addPayApro(query);
                console.log("데이터4 결과값");
                console.log(data4);
                if(!returnAffect(data4[0],"add"))return false;
                if(!returnAffect(data4[1],"update"))return false;
        ...
        // 금액다를경우 
            ...
            var data5 = await webViewModel.updatePayReqState(query);
        ...
        if(error.chk=="addPayApro"){
            ...
            var data7 = await webViewModel.updatePayReqState(query);
            ...
            query.state = "DB에러(일반)";
            var data8 = await webViewModel.updatePayReqState(query);
            res.redirect("/v1/webView/tossFail?result="+"dataBase"+"&msg="+msg);
        }     
    }
